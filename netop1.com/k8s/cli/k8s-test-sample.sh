#!/bin/bash
# Run this script in cli pod to create test channel and test a sample chaincode
# usage: k8s-test-sample.sh

# copy chaincode to GOPATH to prepare for installation and test
cp -R /etc/hyperledger/cli/config/chaincode /opt/gopath/src/github.com

cd /etc/hyperledger/cli/config/artifacts
